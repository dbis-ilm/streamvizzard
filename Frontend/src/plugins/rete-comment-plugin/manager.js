...
export default class CommentManager {
    ...
    addFrameComment(text, [ x, y ], links = [], width = 0, height = 0,
                    collapsed = false, triggerCollapse = false) {
        ...
        //Initialize collapse
        comment.instance.setCollapsed(collapsed, triggerCollapse);
    ...
    getComment(id) {
        return this.comments.filter(c => c.getID() === id)[0];
    }
    ...
    fromJSON(list) {
        ...
        list.map(item => {
            if (item.type === 'frame') {
                this.addFrameComment(item.text, item.position, item.links, item.width, item.height, item.collapsed)
            } else {
