...
kafkaSource1Def = KafkaSource.builder() \
...
kafkaSource2Def = KafkaSource.builder() \
    .set_bootstrap_servers("kafka:9093") \
...
class ProcessAllWindow(ProcessAllWindowFunction):
    def process(self, _, elements):
        yield list(elements)


class LSTM(MapFunction):
    def __init__(self):
    ...
    def map(self, tupleIn):
        data = [json.loads(d) for d in tupleIn]
...
def processRes(inTuple):
    dataLstm = inTuple[0][0]
    dataCnn = json.loads(inTuple[1])
...
KafkaSource1 = env.from_source(kafkaSource1Def, WatermarkStrategy.for_monotonous_timestamps(), 'kafkaSource1', type_info=Types.STRING()).set_parallelism(1).assign_timestamps_and_watermarks(WatermarkStrategy.for_monotonous_timestamps().with_idleness(Duration.of_seconds(10)).with_timestamp_assigner(EventTimeAssigner())).set_parallelism(1)

KafkaSource2 = env.from_source(kafkaSource2Def, WatermarkStrategy.for_monotonous_timestamps(), 'kafkaSource2', type_info=Types.STRING()).set_parallelism(1).assign_timestamps_and_watermarks(WatermarkStrategy.for_monotonous_timestamps().with_idleness(Duration.of_seconds(10)).with_timestamp_assigner(EventTimeAssigner())).set_parallelism(1)
Windowed = KafkaSource2.window_all(CountTumblingWindowAssigner.of(40)).process(ProcessAllWindow(), output_type=Types.LIST(Types.LIST(Types.STRING()))).set_parallelism(1)
LSTMOp = Windowed.map(LSTM(), output_type=Types.LIST(Types.LIST(Types.DOUBLE()))).set_parallelism(5)

JoinOp = LSTMOp.connect(KafkaSource1).key_by(lambda x: 1, lambda x: 1).process(StreamBufferedOrderedJoinFunction(env.get_config().get_auto_watermark_interval()), output_type=Types.TUPLE([Types.LIST(Types.LIST(Types.DOUBLE())), Types.STRING()])).set_parallelism(1)
ProcessRes = JoinOp.map(processRes, output_type=Types.TUPLE([Types.DOUBLE(), Types.DOUBLE()])).set_parallelism(1)
FileSink = ProcessRes.sink_to(fileSinkDef).set_parallelism(1)
