...
export default class _ReadFolder extends Definition {
    ...
    build(operator) {
        ...
        let path = new StringParam("path", "", "Path", "Source folder to read files from, ordered by modification date");
        let limitRate = new BoolParam("limitRate", true, "Limit Rate","If the source should produce tuples in a fixed rate");
        let rate = new NumberParam('rate', 30, 0, null, "Rate","How many lines per second are processed");
