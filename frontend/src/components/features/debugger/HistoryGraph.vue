...
class HistoryBranch {
  ...
  registerDisplayElement(element) {
  ...
  updatePipelineUpdates() {
    for(let idx = this.updates.length - 1; idx >= 0; idx--) {
      let s = this.updates[idx];

      if(s.stepTime < this.startTime) {
        s.destroy();
        this.updates.splice(idx, 1);
      } else s.updateXPosition(this);
    }
  }

  updateDisplayElements() {
    for(let idx = this.displayElements.length - 1; idx >= 0; idx--) {
      let s = this.displayElements[idx];

      if(s.stepTime < this.startTime) {
        s.destroy();
        this.displayElements.splice(idx, 1);
      } else s.updateXPosition(this);
    }
  }
  ...
  clearDisplayElements() {
    for(let [, value] of Object.entries(this.displayElements)) value.destroy();
...
export const HistoryGraphDisplayElementType = {
  StepHighlight: 0,
  UpdateHighlight: 1,
};

class HistoryGraphDisplayElement {
  constructor(branch, stepID, stepTime, displayType, data) {
    ...
    this.element.setAttribute("class", "historyGraphDisplayElement");
    this.element.setAttribute("title", data);
    this.element.setAttribute("branchID", branch.id);
    this.element.setAttribute("stepID", stepID);
    branch.container.appendChild(this.element);
  ...
  updateXPosition(branch) {
    ...
    let timeRange = branch.timeRange();
    let xOff = timeRange > 0 ? Math.min(100,  (Math.max(0, (this.stepTime - branch.startTime)) / timeRange) * 100) : 0;
    this.element.style.left = "calc(" + xOff + "% - 3px)" // Offset half the width to center at correct position
  }
...
export default {
  ...
  mounted() {
    ...
    $(this.$el).on('click', '.historyGraphPipelineUpdate,.historyGraphDisplayElement', async function(event){
      event.stopPropagation();
  ...
  methods: {
    ...
    clearDisplayElements() {
      for(let [, branch] of Object.entries(this.branches)) branch.clearDisplayElements();
    },

    registerGraphDisplayElements(displayType, branchID, stepID, stepTime, data) {
      let branch = this.branches[branchID];

      if(branch == null) return;

      let de = new HistoryGraphDisplayElement(branch, stepID, stepTime, displayType, data);

      branch.registerDisplayElement(de);
    },
    ...
    updateBranchData(branchID, branchStartTime, branchEndTime, stepCount, stepOffset, deleteOnEmpty=false) {
      ...
      if(stepCount === 0 && deleteOnEmpty) this._removeBranch(branchID);
    },
...
.historyGraphDisplayElement::after {
  content: '';
  background: linear-gradient(90deg, rgba(255,255,255,0) 0%, rgb(255, 255, 0) 50%, rgba(255,255,255,0) 100%);
