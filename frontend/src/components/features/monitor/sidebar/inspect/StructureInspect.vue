<template>
  <div class="inspectContainer" ref="container">
    <div ref="omitted" class="disabled noSelect" v-if="omitted > 0" style="margin-bottom: 10px;"><i>[{{ omitted }} element(s) were omitted]</i></div>
  </div>
</template>
...
import $ from 'jquery'
import Vue from "vue";
import StructureInspectElement from "@/components/features/monitor/sidebar/inspect/StructureInspectElement.vue";
  ...
  methods: {
    ...
    buildElements(root, container) {
      ...
      if(typeof root === "object" && !Array.isArray(root)) { //Dict
        ...
        for(let i=0; i < Math.min(this.maxDisplayElms, root.length); i++) {
          let c = this._createElement("[" + i + "]", container);

          this.buildElements(root[i], c);
        }

        this.omitted = Math.max(0, root.length - this.maxDisplayElms);
      }
    },
